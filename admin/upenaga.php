...
<body>
	...
	<header>
		...
		<center>
			<form method="post" name="result" action="upenaga.php">
				<table class="table table-hover">
                    <tr>
						<td>IC No.:</td>
                        <td><input class="form-control" placeholder="search" name="p_ic"
		...
		<?php
		 $bil=1;
			if(isset($_POST['p_ic'])) {
				$p_ic = $_POST['p_ic'];
				$sql = "SELECT * FROM penaga WHERE p_ic LIKE '$p_ic%'";
			} else {
				$sql = "SELECT * FROM penaga";
					}
        ...
        <tr>
            ...
            <td><a href="pcert.php?id_p=<?php echo $row['id_p']; ?>" onclick="return sahkan('view')">
            <img src="view.png" style="width: 30px" title="view"></a></td>
            
            <td><a href="penaga_edit.php?id_p=<?php echo $row['id_p']; ?>" onclick="return sahkan('update')">
            <img src="edit.png" style="width: 30px" title="update"></a></td>
			
			<td><a href="penaga_del.php?id_p=<?php echo $row['id_p']; ?>" onclick="return confirm('Are you sure you want to delete?')">
